...
class _MiniMapState extends State<MiniMap> {
  ...
  void _updateOverlayPosition() {
    ...
    setState(() {
      if (widget.mainScrollController.position.maxScrollExtent > 0) {
        ...
        // Calculate available space for the overlay to move
        final double availableHeight =
            MediaQuery.of(context).size.height - widget.overlayHeight;
  ...
  void _handleMiniMapTap(TapDownDetails details) {
    ...
    if (!isOnOverlay) {
      ...
      final double targetCenter = tapPosition.clamp(
        ...
        maxHeight - (widget.overlayHeight / 2),
      );
  ...
  void _handleMiniMapDragUpdate(DragUpdateDetails details) {
    ...
    if (tapPosition >= 0 && tapPosition <= MediaQuery.of(context).size.height) {
      final double maxHeight = MediaQuery.of(context).size.height;

      // Calculate center position for the overlay
      final double targetPosition = (tapPosition - (widget.overlayHeight / 2))
          .clamp(0.0, maxHeight - widget.overlayHeight);
  ...
  void _scrollBasedOnOverlayPosition() {
    ...
    final double maxPosition =
        MediaQuery.of(context).size.height - widget.overlayHeight;
    final double scrollPercent = _overlayPosition / maxPosition;
        ...
        SizedBox(
          ...
          child: Stack(
            children: [
              ...
              // Transparent overlay for background interaction (MOVED BELOW DRAGGABLE OVERLAY)
              Positioned.fill(
              ...
              // Viewport indicator overlay - draggable (MOVED ABOVE BACKGROUND INTERACTION LAYER)
              Positioned(
                left: 4,
                right: 4,
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: widget.overlayColor, width: 2),
