...
var ImageGenTool = utils.AITool{
	...
	Exec: func(args string) string {
		...
		if apiKey == "" {
			return "TOGETHER_API_KEY is not set"
		}
		...
		if err != nil {
			return fmt.Sprintf("Error marshaling request: %s", err.Error())
		}
		...
		if resp.StatusCode != http.StatusOK {
			return fmt.Sprintf("Image generation failed with status %d: %s", resp.StatusCode, string(body))
		}
		...
		// Return in the format expected by client's MessageContent.complexTools()
		...
		if err != nil {
			return fmt.Sprintf("Error marshaling result: %s", err.Error())
		}
