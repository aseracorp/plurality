...
var WebTool = utils.AITool{
	...
	Exec: func(input string, _ utils.Conversation) utils.MessageContent {
		// Parse the URL parameter from JSON
		var params map[string]string
		err := json.Unmarshal([]byte(input), &params)
		if err != nil {
			return utils.NewTextContent(fmt.Sprintf("Error parsing URL parameter: %s", err.Error()))
		}

		url := params["URL"]
		if url == "" {
			return utils.NewTextContent("No URL provided")
		}
		...
		if err != nil {
			return utils.NewTextContent(fmt.Sprintf("Error visiting URL: %s", err.Error()))
		}
		defer resp.Body.Close()

		// Check status code
		if resp.StatusCode != http.StatusOK {
			return utils.NewTextContent(fmt.Sprintf("Error: received status code %d when visiting %s", resp.StatusCode, url))
		}

		// Read body
		body, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			return utils.NewTextContent(fmt.Sprintf("Error reading response body: %s", err.Error()))
		}

		// Parse the HTML
		doc, err := goquery.NewDocumentFromReader(strings.NewReader(string(body)))
		if err != nil {
			return utils.NewTextContent(fmt.Sprintf("Error parsing HTML: %s", err.Error()))
		}
		...
		return utils.NewTextContent(result)
	},
}
