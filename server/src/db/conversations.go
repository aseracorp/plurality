...
func UpdateConversationState(ctx context.Context, conversationID primitive.ObjectID, state utils.ConversationState) error {
	client := GetClient()
	collection := client.Database("plurality").Collection("conversations")
	userID, ok := ctx.Value("userID").(string)
...
// GetActiveConversationsForUser returns conversations that are not idle (processing or waiting for tool).
func GetActiveConversationsForUser(ctx context.Context) ([]utils.Conversation, error) {
	...
	cursor, err := collection.Find(ctx, bson.M{
		"user_id": userID,
		"state":   bson.M{"$ne": utils.StateIdle},
	})
	if err != nil {
		return nil, err
	}
